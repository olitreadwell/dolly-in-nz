      ...
      <ol className="mt-14 border-l border-gold/30">
        {timelineEntries.map((entry, index) => (
          <li key={entry.year + entry.title} className="relative pb-12 pl-10 last:pb-0">
            ...
            <Reveal>
              <p className="font-display text-2xl italic text-gold">{entry.year}</p>
              <h3 className="mt-2 text-xl font-semibold text-cream">{entry.title}</h3>
              <p className="mt-2 max-w-[58ch] leading-relaxed text-moss">{entry.body}</p>
